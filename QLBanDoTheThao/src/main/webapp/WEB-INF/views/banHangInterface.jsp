...
<body>
	...
	<main class="container  ">
		...
		<div class="bg-light p-5 rounded">
			...
			<h2 style="font-size: 20px">DANH SÁCH SẢM PHẨM</h2>
			${message}
			<useBean id="pagedListHolder" scope="request"
				type="org.springframework.beans.support.PagedListHolder" />
			<c:url value="/banhang.htm" var="pagedLink">
		...
		<div class="bg-light p-5 rounded">
			...
			<form class="d-inline-flex">
				<input name="searchKH" id="searchInputBH" class="form-control me-2"
					type="search" placeholder="Search customer" aria-label="Search">


				<button name="btnsearchKH" id="searchProduct"
					class="btn btn-outline-success" type="submit">Search</button>
			...
			<form class="row g-3" action="banhang.htm">
				<div class="col-md-4">
					<label for="exampleFormControlInput1" class="form-label">Mã
						khách hàng </label> <input name="maKH" type="text" class="form-control"
						id="exampleFormControlInput1"
						placeholder="Vui lòng nhập mã khách hàng" value="${maKH}" />
				</div>
				<div class="col-md-4">
					<label for="exampleFormControlInput1" class="form-label">Tên
						khách hàng</label> <input name="tenKH" type="text" class="form-control"
						id="exampleFormControlInput1"
						placeholder="Vui lòng nhập tên khách hàng" value="${tenKH}" />
				</div>


				<div class="col-md-4">
					<label for="exampleFormControlInput1" class="form-label">Khuyến
						mãi</label> <select name="comboboxKM"
						...
						<c:forEach items="${listKM}" var="km">
							<option value="${km.makm}"
								class="form-select form-select-lg mb-6"
								aria-label=".form-select-lg example">${km.tenkm}</option>
						</c:forEach>
					</select>
